...
Como variante condicionada, puede considerarse una preselección liviana y conservadora cuando exista una justificación experimental clara. Por ejemplo, un criterio de movimiento o un detector cerrado liviano podrían utilizarse para priorizar segmentos candidatos antes de enviarlos al CPN. Esta variante introduce riesgo de falsos negativos antes de la inferencia OVD; por ello, debe declararse en la configuración de corrida, instrumentar descartes y compararse, cuando corresponda, contra un flujo sin preselector. **Actualización 2026-07-15:** esta variante ya está implementada sobre la OAK-D Pro PoE como opción de corrida, apagada por defecto (`docs/_archive/superpowers/specs/2026-07-15-oak-d-prefilter-en2-design.md`): un detector cerrado de personas on-device reenvía el frame al CPN solo con evidencia reciente de persona, heartbeat periódico o silencio del propio detector (fail-open), y cada corrida declara descartes y config efectiva en `summary.json`. La inferencia OVD sigue concentrada en el CPN (EN-3 permanece fuera de alcance); en la topología two-node los contadores de descarte todavía no viajan al nodo de referencia y quedan declarados no disponibles en esa variante (v1).
...
| EN-2: preselección liviana conservadora | El EN utiliza criterios simples, movimiento o un modelo cerrado liviano para priorizar segmentos candidatos. | Implementada como variante opcional de corrida, apagada por defecto (spec 2026-07-15): detector de personas on-device con fail-open, registro de descartes en `summary.json`; contadores no disponibles en two-node v1. |
