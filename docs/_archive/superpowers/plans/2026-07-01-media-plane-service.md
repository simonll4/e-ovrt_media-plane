...
- **Limitación conocida (hardening Fase 2, no bug)**: `RunEventBus._queues` (Task 9) nunca elimina la cola de un run terminado — crece una entrada por run durante la vida del proceso. Es intencional para Fase 1 (permite que un WS se conecte tarde y reciba los eventos bufferizados) y negligible dado el uso single-user con un run activo a la vez y reinicio del proceso para cambiar de modelo. Fase 2 (servicio de larga vida, multi-nodo) debería agregar limpieza de colas de runs viejos (p.ej. TTL o tope de N runs retenidos).
- **Contrato con el BFF (Spec B)**: el cuerpo de `POST /api/runs` (`CreateRunRequest`: `ingest{plugin,ref,config}` + `prompts{prompt_set,active_ids}` + `run{stride,max_units,save_annotated_video,min_confidence}`), las respuestas (`RunStatusResponse`, `ModelInfoResponse`, `IngestPluginResponse`, `DatasetCatalogEntryResponse`) y los eventos WS (`started`/`progress`/`finished`) fueron verificados campo a campo contra lo que el BFF envía/espera en `2026-07-01-webconsole-backend.md` (`build_service_run_request`, `RunBackend`, `fake_service.py`). El fake service del BFF es permisivo (acepta `dict`), así que la equivalencia exacta del contrato la garantiza esta revisión de diseño, no los tests del BFF por sí solos.
> **Nota (2026-07-03):** las dos observaciones anteriores describen el diseño **superseded** (RunEventBus, `CreateRunRequest`). El contrato canónico implementado es el del plan vigente `2026-07-03-media-plane-service-fase1.md` (`RunRequest{ingest{plugin,config}, prompts{set_inline,active_ids}}`, `EventBroadcaster`). La cross-validación con el plan de Spec B debe re-hacerse contra ese contrato al escribir el plan de Spec B.
